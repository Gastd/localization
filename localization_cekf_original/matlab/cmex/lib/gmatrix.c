...
**********************************************************************************************/
/** 
 *  PGMATRIX_SETSIZE ï¿½ uma funï¿½ï¿½o...
 */
...
#if GMATRIX_DEFINE_SUBTRACT_COPY
void PGMATRIX_SUBTRACT_COPY(PGMATRIX pMatResult, PGMATRIX pMatA, PGMATRIX pMatB)
{
	int i,j;
	GMATRIX_ASSERT("GMATRIX_SUBTRACT_COPY",pMatA->Nr != pMatB->Nr);	
	GMATRIX_ASSERT("GMATRIX_SUBTRACT_COPY",pMatA->Nc != pMatB->Nc);	
	PGMATRIX_SETSIZE(pMatResult,pMatA->Nr, pMatA->Nc);
	for(i=1;i<=pMatA->Nr;++i){	
		for(j=1;j<=pMatA->Nc;++j){	
			PGMATRIX_DATA(pMatResult,i,j) = PGMATRIX_DATA(pMatA,i,j) - PGMATRIX_DATA(pMatB,i,j); 
...
#if GMATRIX_DEFINE_SUBTRACT_IDENTITY_COPY
void PGMATRIX_SUBTRACT_IDENTITY_COPY(PGMATRIX pMatResult, PGMATRIX pMat)
{
	int i,j;
	GMATRIX_ASSERT("GMATRIX_SUBTRACT_COPY",pMat->Nr != pMat->Nc);	
	PGMATRIX_SETSIZE(pMatResult,pMat->Nr, pMat->Nc);
	for(i=1;i<=pMat->Nr;++i){	
		for(j=1;j<=pMat->Nc;++j){
			if (j==i){
...
#if GMATRIX_DEFINE_SUBTRACT_IDENTITY
...
#if GMATRIX_DEFINE_MULTIPLY_SUBTRACT
void PGMATRIX_MULTIPLY_SUBTRACT(PGMATRIX pMatResult, PGMATRIX pMatA, PGMATRIX pMatB) 
{
	...
	GMATRIX_ASSERT("PGMATRIX_MULTIPLY_SUBTRACT",pMatA->Nc != pMatB->Nr);	
	GMATRIX_ASSERT("PGMATRIX_MULTIPLY_SUBTRACT",pMatResult->Nr != pMatA->Nr);	
	GMATRIX_ASSERT("PGMATRIX_MULTIPLY_SUBTRACT",pMatResult->Nc != pMatB->Nc);	
	for(i=1;i<=pMatA->Nr;++i){	
		for(j=1;j<=pMatB->Nc;++j){	
			Acc = 0.0;
			for(k=1;k<=pMatA->Nc;++k){	
