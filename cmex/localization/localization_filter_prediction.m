...
for i=1:DimX,
   ...
   YSamples(:,i)      = localization_filter_model_f('evaluate',XSamples(1:kf_structure.Nstates,i),XSamples((kf_structure.Nstates+1):(kf_structure.Nstates+6),i),G,T,kf_structure.flagestimateaccelerometerbias);
   YSamples(:,i+DimX) = localization_filter_model_f('evaluate',XSamples(1:kf_structure.Nstates,i+DimX),XSamples((kf_structure.Nstates+1):(kf_structure.Nstates+6),i+DimX),G,T,kf_structure.flagestimateaccelerometerbias);
end,
WSamples(1,UtNSamples) = kappa/(DimX+kappa);
XSamples(:,UtNSamples) = Xm;
YSamples(:,UtNSamples) = localization_filter_model_f('evaluate',XSamples(1:kf_structure.Nstates,UtNSamples),XSamples((kf_structure.Nstates+1):(kf_structure.Nstates+6),UtNSamples),G,T,kf_structure.flagestimateaccelerometerbias); 
