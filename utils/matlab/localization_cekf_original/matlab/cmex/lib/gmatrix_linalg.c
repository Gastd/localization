...
double PGMATRIX_NORM(PGMATRIX pMat)
{
	...
	GMATRIX_ASSERT("PGMATRIX_NORM",(pMat->Nr != 1) && (pMat->Nc != 1));	// must be a vector
	GMATRIX_ASSERT("PGMATRIX_NORM",pMat->Nr <= 0);	
	GMATRIX_ASSERT("PGMATRIX_NORM",pMat->Nc <= 0);	

	if(pMat->Nr==1){
		for (j=1; j<=pMat->Nc; ++j){ 
			Norm += PGMATRIX_DATA(pMat,1,j)*PGMATRIX_DATA(pMat,1,j); 
		} 
	}
	else{
		for (j=1; j<=pMat->Nr; ++j){ 
			Norm += PGMATRIX_DATA(pMat,j,1)*PGMATRIX_DATA(pMat,j,1); 
		} 
	}
...
void PGMATRIX_DERIVATIVE_LEFT_PSEUDOINVERSE(PGMATRIX pdApinvdx, PGMATRIX pdAdx, PGMATRIX pA, PGMATRIX pApinv, PGMATRIX pMatDummy1, PGMATRIX pMatDummy2)
{
	...
	PGMATRIX_SUBTRACT_IDENTITY(pMatDummy1); // MatDummy1 = A*Apinv-I;
